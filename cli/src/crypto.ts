...
export function eciesEncrypt(data: Buffer, recipientPublicKey: Buffer): string {
  ...
  const ephemeralPublicRaw = ephemeral.publicKey
    ...
    .subarray(-RAW_KEY_LENGTH);

  const derivedKey = deriveSharedKey(
    ephemeral.privateKey,
    toX25519PublicKeyObject(recipientPublicKey),
  );
...
export function eciesDecrypt(envelope: string, privateKeyRaw: Buffer): Buffer {
  ...
  const derivedKey = deriveSharedKey(
    toX25519PrivateKeyObject(privateKeyRaw),
    toX25519PublicKeyObject(ephemeralPublicRaw),
  );
...
export function savePrivateKey(serverHost: string, privateKeyRaw: Buffer) {
  ...
  fs.writeFileSync(getPrivateKeyPath(serverHost), privateKeyRaw.toString("base64"), {
    mode: 0o600,
  });
}
...
export function derivePublicKey(privateKeyRaw: Buffer): Buffer {
  const publicKeyDer = crypto
    .createPublicKey(toX25519PrivateKeyObject(privateKeyRaw))
